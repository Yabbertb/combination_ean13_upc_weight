/* global currency, currencyRate, currencySign, currencyFormat, currencyBlank, contentOnly,
   ...
   productEan13, productUpc, productWeight, productAvailableForOrder, productPriceWithoutReduction, productUnitPriceRatio, quantityAvailable,
  ...
  if (typeof contentOnly !== 'undefined') {
    ...
    if (!contentOnly && !!$.prototype.fancybox) {
      $('span.fancybox, a.fancybox').fancybox({
        ...
        afterShow: function() {
          var myButtons = setTimeout(function() {
            $("#fancybox-buttons")
            .clone(true, true)
            .attr("class", "clone")
        ...
        onPlayStart: function() {
          $("#fancybox-buttons.clone").find(".btnPlay").attr('title', FancyboxI18nPause).addClass('btnPlayOn');
        },
        onPlayEnd: function() {
          $("#fancybox-buttons.clone").find(".btnPlay").attr('title', FancyboxI18nStart).removeClass('btnPlayOn');
        },
        onUpdate: function() {
          var toggle;
...
function updateDisplay() {
  ...
  if (!selectedCombination['unavailable'] && quantityAvailable > 0 && productAvailableForOrder == 1) {
    ...
    if (quantitiesDisplayAllowed) {
      ...
      $('#quantityAvailable').html('<strong>' + quantityAvailable + '</strong>');
    ...
    $('#pQuantityAvailable:visible').show();
    $('#quantityAvailable').html('<strong>' + quantityAvailable + '</strong>');
  ...
  var $productEan13 = $('#product_ean13');
  if (selectedCombination['ean13'] || productEan13) {
    if (selectedCombination['ean13']) {
      $productEan13.find('span').text(selectedCombination['ean13']);
    } else if (productEan13) {
      $productEan13.find('span').text(productEan13);
    }
    if (selectedCombination['ean13'] != 0 || productEan13 != 0) {
      $productEan13.show();
    }
  } else {
    $productEan13.hide();
  }
  var $productUpc = $('#product_upc');
  if (selectedCombination['upc'] || productUpc) {
    if (selectedCombination['upc']) {
      $productUpc.find('span').text(selectedCombination['upc']);
    } else if (productUpc) {
      $productUpc.find('span').text(productUpc);
    }
    $productUpc.show();
  } else {
    $productUpc.hide();
  ...
  if (selectedCombination['weight'] || productWeight) {
    if (selectedCombination['weight']) {
      productWeightSum = Number(productWeight) + Number(selectedCombination['weight']);
      $productWeight.find('span').text(number_format(productWeightSum));
    } else if (productWeight) {
      $productWeight.find('span').text(number_format(productWeight));
    }
    $productWeight.show();
  } else {
    $productWeight.hide();
...
// Language-dependent weight format
function number_format(number) {
  if (number == null || !isFinite(number)) {
    throw new TypeError("Number is not valid: " + number);
  }
  number = new Intl.NumberFormat(langIso, {minimumFractionDigits: 3}).format(number);
  return number;
}
...
$(document).ready(function() {
  ...
  var itemWidth = "";
  $('.left-lst, .right-lst').click(function() {
    var condition = $(this).hasClass("left-lst");
    if (condition) {
      click(0, this);
    } else {
      click(1, this)
  ...
  function ResCarouselSize() {
    ...
    $(itemsDiv).each(function() {
      ...
      $(".left-lst").addClass("over");
    });
  }
  function ResCarousel(e, el, s) {
    var leftBtn = ('.left-lst');
    var rightBtn = ('.right-lst');
    var translateXval = '';
...
$(document).ready(function() {
  var itemsMainDiv = ('.pamulti-carousel');
  var itemsDiv = ('.pamulti-carousel-inner');
  var itemWidth = "";
  $('.paleft-lst, .paright-lst').click(function() {
    var condition = $(this).hasClass("paleft-lst");
    if (condition) {
      click(0, this);
    } else {
      click(1, this)
  ...
  function ResCarouselSize() {
    ...
    $(itemsDiv).each(function() {
      id = id + 1;
      var itemNumbers = $(this).find(itemClass).length;
      $(this).parent().attr("id", "pamulti-carousel" + id);
      if (bodyWidth >= 1200) {
        itemWidth = sampWidth / 5;
        if (itemNumbers <= 5) {
          $(".paright-lst").addClass("over");
        }
      } else if (bodyWidth >= 992) {
        itemWidth = sampWidth / 4;
        if (itemNumbers <= 4) {
      ...
      $(".paleft-lst").addClass("over");
    });
  }
  function ResCarousel(e, el, s) {
    var leftBtn = ('.paleft-lst');
    var rightBtn = ('.paright-lst');
    var translateXval = '';
    var divStyle = $(el + ' ' + itemsDiv).css('transform');
    var values = divStyle.match(/-?[\d\.]+/g);
    var xds = Math.abs(values[4]);
